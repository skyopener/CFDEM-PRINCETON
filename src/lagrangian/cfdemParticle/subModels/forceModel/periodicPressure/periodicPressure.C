...
void periodicPressure::setForce() const
{
	...
	particleCloud_.mesh(),
        dimensionedVector("source", dimensionSet(1,-2,-2,0,0), domainPartVolFraction*rhoParticle_*g_.value() + totalFluidWeight/volDomain) 
    );
    ...
    for(int index = 0;index <  particleCloud_.numberOfParticles(); index++)
    {
        ...
        //{
		    ...
		    DEMForces()[index][j] -= Volp*source_[cellI][j];
            	...
            	if(verbose_ && index >=0 && index <1)  Pout << "index = " << index << " Static pressure gradient force ("
                                                            << DEMForces()[index][0] << " "
                                                            << DEMForces()[index][1] << " "
