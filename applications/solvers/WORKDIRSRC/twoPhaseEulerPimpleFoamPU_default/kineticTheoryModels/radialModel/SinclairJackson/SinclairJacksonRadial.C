...
{
    return 
        (-1.0/3.0)*pow(max(alpha, 0.0001)/alphaMax, -2.0/3.0)
       /(alphaMax*sqr(1.0 - pow(max(alpha, 0.0001)/alphaMax, 1.0/3.0)));
}

//_AO_09/08/2014
Foam::tmp<Foam::volScalarField> Foam::SinclairJacksonRadial::g0jamming
(
    const fvMesh& mesh,
    const volScalarField& alpha,
    const dimensionedScalar& alphaMax,
...
Foam::tmp<Foam::volScalarField> Foam::SinclairJacksonRadial::g0jammingPrime
(
    const fvMesh& mesh,
    const volScalarField& alpha,
    const dimensionedScalar& alphaMax,
